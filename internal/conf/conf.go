...
import (
	...
	"github.com/G-Node/gogs/internal/user"
)
...
func Init(customConf string) error {
	...
	if err = File.Section("http").MapTo(&HTTP); err != nil {
		...
		log.Fatal("Failed to map Search settings: %v", err)
	} else if err = File.Section("doi").MapTo(&DOI); err != nil {
		log.Fatal("Failed to map DOI settings: %v", err)
	} else if err = File.Section("cliconfig").MapTo(&CLIConfig); err != nil {
		log.Fatal("Failed to map Client config settings: %v", err)
	} else if err = File.Section("dav").MapTo(&WebDav); err != nil {
		log.Fatal("Failed to map WebDav settings: %v", err)
	}
